...
def attention_layer(from_tensor,
    ...
    # TODO: Investigate why we're getting underflow in softmax
    # Help with stability when using mixed precision!
    if compute_type == tf.float16:
        attention_scores = tf.cast(attention_scores, tf.float32)
    # attention_scores = tf.maximum(attention_scores, 1e-12)
    attention_probs = tf.nn.softmax(attention_scores)
    if compute_type == tf.float16:
        attention_probs = tf.saturate_cast(attention_probs, tf.float16)
